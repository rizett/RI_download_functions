...
%----------------------------------------------------------------------------
%%% ABOUT %%
% This function extracts the time, lat, long, and gridded data from 
...
% 
% [dat]=extract_ncep(ndir,latr,lonr,type);
% 
% INPUT:
...
%--- load the file, and extract the data
    for kk = 1:numel(fname)
        if kk == 1; %get time, lat, long only on first iteration
            %time
            ...
            %lat / long
                ...
                long = double(ncread(fname(kk).name,'lon'));
                ...
                %Find lat / long data within specified latr / lonr
                    ...
                    lo = find(long >= lonr(1) & long <= lonr(2));
