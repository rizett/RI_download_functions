...
% the speified year to a specified or default directory. 
...
%--- CD to direcotry where data will be saved
    if nargin == 1
        if ispc
            cd 'C:\Users\Robert\Desktop\Data\NCEP' %default directory if none specified
        else
            cd '/Volumes/1 RIzett SG/RI/Data/NCEP' %default directory if none specified
        end
...
%--- Make a list of data files to download 
    ...
    if dload(1) == 1;
        ...
        fprintf(fid,'%s\n',['ftp://ftp2.psl.noaa.gov/Datasets/ncep.reanalysis2/gaussian_grid/uwnd.10m.gauss.', num2str(year),'.nc']);
    end
    if dload(2) == 1;
        % fprintf(fid,'%s\n',['ftp://ftp.cdc.noaa.gov/Datasets/ncep.reanalysis/surface_gauss/vwnd.10m.gauss.', num2str(year),'.nc']);
        fprintf(fid,'%s\n',['ftp://ftp2.psl.noaa.gov/Datasets/ncep.reanalysis2/gaussian_grid/vwnd.10m.gauss.', num2str(year),'.nc']);
    end
    %Air T
    if dload(3) == 1;
...
%--- Download the data through command line using wget
    if ~all(dload == 0)
        ...
        %IF Mac
        %Do not use system command to cd. The following system(wget)
        %command will download files to the current Matlab directory 
        %(which is set to ndir ... see above)
